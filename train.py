...
scheduler = StepLR(optimizer, step_size=args.step_size_lr,
                   gamma=args.decay_lr)  # Dynamically changing the learning rate
mask = utils.get_mask()  # Getting the mask
softmax = nn.Softmax()
...
def run_simulation():
    def train():
        ...
        X_tweet_features, X_network_features, X_user_context_features, seq_lengths, Y = \
            utils.get_batch(size_comp=int(args.batch_size / 2), size_notcom=int(args.batch_size / 2), mask=mask,
        ...
        for iteration in range(args.iterations):
            ...
            if iteration % 25 == 0 and iteration:
