...
# This function calculates the clustering coefficients according to Equation (2) of E2ECAD paper (https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=8508296)
...
def extract_network_features():
    ...
    # We need to load the adjaceny matrix
    ...
    for i, row in enumerate(id_label_split_records):
        ...
        # For the refernce to the different connections refer to the
...
# This function creates aggregted pickle files ready to be fed into the model
def create_dataset():
    # First we need to load the file containing IDs, Lables, and Split. This file contains Twitter handles though we don't use it for training
    id_label_split_records = pd.read_csv(args.project_dir + 'Data/ID-TwitterHandle-Label-Split.csv',
                                         sep=',', names=["ID", "TwitterHandle", "Label", "Split"])
